...
class AnimatedTruck extends StatefulWidget {
  ...
  const AnimatedTruck({Key? key, required this.primaryColor}) : super(key: key);
...
class MovingWind extends StatefulWidget {
  ...
  const MovingWind({Key? key, required this.primaryColor}) : super(key: key);

  @override
  State<MovingWind> createState() => _MovingWindState();
}

class _MovingWindState extends State<MovingWind>
    with SingleTickerProviderStateMixin {
...
class MovingWindPainter extends CustomPainter {
  ...
  MovingWindPainter(this.progress, this.primaryColor);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var line in windLines) {
      ...
      // Draw wind line
      canvas.drawLine(Offset(x, y), Offset(x - length, y), windPaint);

      // Draw additional segments for longer wind effect
      canvas.drawLine(
        Offset(x - length - 10, y),
        Offset(x - length - 25, y),
        windPaint..color = primaryColor.withOpacity(0.08),
      );
    }
  }
...
class MovingRoad extends StatefulWidget {
  ...
  const MovingRoad({Key? key, required this.primaryColor}) : super(key: key);

  @override
  State<MovingRoad> createState() => _MovingRoadState();
}

class _MovingRoadState extends State<MovingRoad>
    with SingleTickerProviderStateMixin {
...
class MovingRoadPainter extends CustomPainter {
  ...
  MovingRoadPainter(this.progress, this.primaryColor);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (
      double x = size.width + dashWidth + 100 - offset;
      x > -dashWidth - 100;
      x -= totalDashLength
    ) {
      canvas.drawLine(
                ...
                SizedBox(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      Positioned.fill(
                        child: MovingWind(primaryColor: primaryColor),
                      ),
                      ...
                      Positioned(
                        ...
                        left: 0,
                        right: 0,
                        child: MovingRoad(primaryColor: primaryColor),
                      ),
                      ...
                      Positioned(
                        ...
                        child: AnimatedTruck(primaryColor: primaryColor),
                      ),
                    ],
                  ),
                ),
